...
Detail reference for the OpenVINO stub in `~/agents/claude/CLAUDE.local.md`. Container-scoped capability,
project-agnostic. OpenVINO inference on **iGPU + NPU + CPU**, enabled + verified (real compile+infer correct on
each). Paths absolute: `$HOME=/var/home/eturkes/debian` so `~/.local` ≠ the install root below.
...
Device strings `"NPU"` | `"GPU"` | `"CPU"`; run preference + `AUTO:`/`HETERO:` device selection → the stub (`~/agents/claude/CLAUDE.local.md`).
