...
- pattern = per-project `.venv` (py 3.10–3.13) + `numpy` (2.x ok). OpenVINO still resolves from `PYTHONPATH` inside the venv (augments venv `sys.path`) → only numpy (+ pure deps) in the venv, not openvino
...
- fix = pinned Ubuntu IGC 2.30.1 (glibc ≤2.39) dropped into an isolated symlink "farm" of host driver libs; generic libs (libc/libstdc++/libtbb/…) NOT linked → resolve from container → no ABI clash
