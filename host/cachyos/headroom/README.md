...
- **CLI context tools are gone.** `--context-tool` / `--no-context-tool` and `HEADROOM_CONTEXT_TOOL` now hard-error — drop the flag from the launch line (`headroom wrap claude --no-tokensave --1m`); the feature it disabled no longer exists, so behavior is unchanged. On first run `wrap` purges what rtk/lean-ctx *installations of Headroom's own making* left behind: generated `~/.claude/hooks/` scripts, and `~/.local/bin/{rtk,lean-ctx}` only when they symlink into Headroom's managed dir. A hand-rolled setup is out of its reach — `rtk-bin` at `/usr/bin/rtk` plus a `PreToolUse` hook pointing at `$HOME/.claude/rtk-hook-shim` survives untouched (verified: `~/.claude/settings.json` mtime unchanged across the upgrade).
