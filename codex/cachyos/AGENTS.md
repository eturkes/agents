...
- Headless capture: `webcap <url> [--pdf F] [--png F] [--dom F|-]` (`host/cachyos/webcap`, CDP over chromiumfish). Full-page PNG: `--full-page` → inspect directly. Also `--dark`, `--width`/`--height`, `--selector`/`--wait` settle, `--timeout`, `--user-data-dir`. Fragment URLs scroll to their target, which client-routed pages can reset. `--user-data-dir D` opens `D/Default`, so aiming it at `~/.config/browser-os` gets an empty profile rather than the signed-in session (`Local State` → `profile.last_used` names its directory), and the running browser locks the directory → authenticated pages stay on BrowserOS MCP.
