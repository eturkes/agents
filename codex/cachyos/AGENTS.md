...
- Headless capture: `webcap <url> [--pdf F] [--png F] [--dom F|-]` (`host/cachyos/webcap`, CDP over chromiumfish). Full-page PNG: `--full-page` → inspect directly. Also `--dark`, `--width`/`--height`, `--selector`/`--wait` settle, `--timeout`, `--user-data-dir`. Fragment URLs scroll to their target, which client-routed pages can reset.
