...
- Authenticated web: drive `webcap --user-data-dir=/run/host/home/eturkes/.config/browser-os`; leave that browser running, since `webcap` captures against a reflink clone and leaves the profile byte-identical. Always reach that profile through `webcap` alone: it appends `--password-store=gnome-libsecret`, which Chrome's last-duplicate rule makes beat the `--password-store=basic` that playwright-core pins into every launch on every platform, and under `basic` a keyring-encrypted jar empties on open. That pin is an explicit switch, not a detector, so naming the store is the whole fix and no desktop-environment variable substitutes for it. A foreign build opening the profile directly also garbage-collects its BrowserOS component extensions. Without the profile flag, `chromiumfish` = isolated visual QA.
