...
- do-not-read set = paths not worth an agent's tokens, distinct from `.gitignore` (some committed files aren't worth reading, some gitignored ones are). Serena honors `.gitignore` → `.serena/project.yml` `ignored_paths` lists only the *non-gitignored* do-not-read paths; keep synced, maintain even without Serena (committed → Serena agents inherit it).
...
- Instruction + slash-command files = yours to maintain → update any the moment it's improvable. Route durable guidance by scope: project-specific → that project's `.agent/memory.md`; agent-agnostic + project-independent → here (`AGENTS.md`); Claude-specific but project-only → the project-level `CLAUDE.md`.
