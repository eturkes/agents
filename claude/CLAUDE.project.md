...
- Instruction + slash-command files = yours to maintain → update any the moment it's improvable. Route durable guidance to one appropriate scope: global `~/.claude/CLAUDE.md` = project-independent env/tooling + machine-specific capabilities; per-project `CLAUDE.md` = generalized principles + config rules for working within projects; `.claude/rules/<topic>.md` = project-specific law + the sole carrier of what a teammate must hold, refresh-safe against the `CLAUDE.md` template.
- `.claude/rules/` reaches MAIN + every `claude`/`general-purpose` teammate with no brief line, in two tiers: a bare file loads beside `CLAUDE.md` at session start; a file whose frontmatter carries `paths:` (quoted globs, repo-root-relative) loads lazily, injected on the tool result when any role touches a matching file. Conventions, stack decisions, gate invocations + forward-binding lessons all live here — project-wide law bare, area-bound law under `paths:`. Keep the bare tier small; it occupies MAIN's context and every teammate's. The bare tier snapshots at MAIN's session start ⇒ law written mid-session reaches a later teammate through its brief, while the `paths:` tier resolves at touch time and carries same-session edits. `Explore`/`Plan`/`claude-code-guide` receive neither tier → brief them with what they must hold.
...
- A gate backing a durable claim must rerun from committed state; scratch-local validator = temporary encoding → record its regeneration path beside the gate invocation in `.claude/rules/` + schedule the port in `.agent/polish.md`.
...
- Context policy (thresholds + one-window aim: global `CLAUDE.md`): PLANNING + MILESTONE-REVIEW + user-requested tasks run across coherent checkpoints as the harness manages the window (a user-stated bound overrides); autonomous runs (WORK-UNIT) hold the one-window aim.
- Attached state (`.agent/roadmap.md`, `polish.md`) rides every session whole → keep it minimal; closed-milestone detail moves to `.agent/archive/` (outside the set), read on demand. Both are MAIN-owned mutable ledgers written mid-session and teammates work from brief + contract → they stay attached rather than moving to `.claude/rules/`, where a frozen snapshot would read as current.
