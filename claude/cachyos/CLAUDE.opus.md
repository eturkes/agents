...
- Serena user-scope MCP = live-LSP primary (nav/edit/rename/diagnostics; ~70 langs, auto-install); project memory remains canonical. Missing language → `global` LSP marketplace gap plugin (registered here; activate per project). At first touch, add real code + edited formats (markdown/toml/yaml/json) to `.serena/project.yml` `language_servers:`, then restart once (startup-only); keep formats with useful diagnostics. Headroom `wrap` (≥0.30) registers Serena as its code-memory MCP + deletes only its own (ledger-recorded) entry; user-registered entries persist.
