...
- Serena user-scope MCP = live-LSP primary (nav/edit/rename/diagnostics; ~70 langs, auto-install); project memory remains canonical. Missing language → `global` LSP marketplace gap plugin (registered here; activate per project). At first touch, add real code + edited formats (markdown/toml/yaml/json) to `.serena/project.yml` `language_servers:`, then restart once (startup-only); keep formats with useful diagnostics. Headroom code-context ranking = tokensave primary + Serena backup; live LSP stays Serena. headroom≥0.30 `wrap` deletes only its own (ledger-recorded) Serena entry; user-registered entries persist.
...
- Docs mirror `~/Projects/agents/docs/<site>/llms.txt` (scopedcommits.com, agentlanguages.dev) > web fetch. `~/Projects/agents/host/cachyos/update-docs` refreshes agentlanguages.dev; maintain the scopedcommits.com copy in place (site publishes no `llms.txt`).
