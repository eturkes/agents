...
- Scope each compaction-free subagent to finish + return its result within 240K, the bound both contexts share. Budget margin + split large rewrites at section boundaries: a read+rewrite agent can peak ~100K on ~40KB (~37%). Transcripts: `~/.claude/projects/<project>/<session>/subagents/agent-<id>.jsonl` (`.message.usage`). Window exhaustion surfaces on the next request as inline `Prompt is too long`.
