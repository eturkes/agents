...
- `grep`/`find` = CC shell-fn shadow → tweakcc `rg-fff` (**RE2**, relevance-ranked, fuzzy-fallback; see `# fff`), distinct from raw `ugrep -G`/`bfs`. `rg`=`/usr/bin/rg` (unshadowed). System `grep` binary = GNU grep 3.11 (BRE). Byte-exact/clean → `command grep` | `/usr/bin/rg` | `rtk proxy grep`.
...
- Serena (Headroom MCP) = primary LSP: symbol nav/edit, ~70 langs, servers auto-install; memory tools off → use the project's. Missing lang → gap-fill plugin in the `global` LSP marketplace. Both global, zero setup. For code-NAV/context-compression Headroom ranks tokensave PRIMARY + Serena BACKUP (`# tokensave`); Serena stays primary for live LSP edit/rename/diagnostics.
...
- **`find P -name|-iname PAT` [-type f] zero-match** → fuzzy filename suggest (`# rg-fff: 0 exact…` + `name [~approx]`), **exit 0** (find code, ≠ grep's 1); suggest lines omit `./`, real matches keep it. `-type d`/`-exec`/`-delete`/`-mtime`/`-maxdepth`/… stay verbatim (fff skips them → side-effect-safe). Off: same env.
...
- Knobs: `RG_FFF_FIRST=0` = regex→byte-equiv BRE-mirror (literals STAY fff-ranked+marked); `RG_FFF_LOG=<path>` per-call decision log; `RG_FFF_DEBUG=1` regex-defer reason.

# tokensave (Headroom-vendored code-graph MCP)

Headroom's PRIMARY code-context compressor (Serena = BACKUP). 34 langs; answers code-nav from a local semantic graph instead of file reads → token savings. FIRST move for "understand/navigate this code" before Read/grep. Core tools (`tokensave_context`/`search`/`status`) land top-level; the other ~85 arrive DEFERRED → `ToolSearch select:mcp__tokensave__…` to load schemas. Each call PROMPTS per use — outside the allowlist by design.
...
- State (regenerable): per-project `.tokensave/`; re-index `tokensave init` (full) / `sync` (incremental). Indexing respects `.gitignore` + skips files >1MB → a symbol missing from the graph is usually one of those, so check coverage before suspecting the index.
