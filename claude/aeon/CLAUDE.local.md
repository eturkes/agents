...
Container-scoped, project-agnostic. OpenVINO on iGPU + NPU + CPU = enabled + verified → **prefer OpenVINO for applicable local inference**.
...
- Before use: source accel env, then run Python from a NumPy venv (3.10–3.13). Enablement, paths, devices, self-test/maintenance, `intel-accel` internals → **`~/agents/docs/openvino.md`**.
