...
- `grep` = CC shell-fn shadow → `rg-fff` (**RE2**, relevance-ranked, fuzzy-fallback; see `# fff`), distinct from raw `ugrep -G`; `find` = RTK hook rewrite → `rtk find`. `rg`=`/usr/bin/rg` (unshadowed). System `grep` binary = GNU grep (BRE). Byte-exact/clean → `command grep` | `/usr/bin/rg` | `rtk proxy grep`.
...
- Session-scratchpad writes via Bash heredoc (`V=<scratchpad> && mkdir -p "$V" && cat > "$V/f" <<'EOF'`) can be permission-DENIED despite the scratchpad's no-prompt design (cause unisolated — heredoc/compound shape suspected) → prefer fileless construction (inline data as program args) or the Write tool.
...
- **Output ≠ real grep** → RANKED by relevance over file-order, strips leading `./`, inline markers, lines capped 512B. Clean/order-stable/byte-exact/full-line → `command grep` (=GNU BRE, `./`, file-order, untruncated) | `/usr/bin/rg` | `rtk proxy grep`.
