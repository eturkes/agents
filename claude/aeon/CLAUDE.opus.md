...
- Python entry points: system → `python3`; uv project → `uv run python`.
...
- `pgrep -f`/`pkill -f` can self-match their `bash -c 'eval …'` wrapper → use one bracketed pattern (`index[.]js`) + `|| echo none` per command; separate kill/relaunch calls.
...
- Serena user-scope MCP = live-LSP primary (nav/edit/rename/diagnostics; ~70 langs, auto-install); project memory remains canonical. Missing language → `global` LSP marketplace gap plugin. At first touch, add real code + edited formats (markdown/toml/yaml/json) to `.serena/project.yml` `language_servers:`, then restart once (startup-only); keep formats with useful diagnostics. Headroom code-context ranking = tokensave primary + Serena backup; live LSP stays Serena. headroom≥0.30 `wrap` removes its own Serena entry but preserves user entries → keep user registration; restore: `claude mcp add -s user serena -- uvx --from git+https://github.com/oraios/serena serena start-mcp-server --project-from-cwd --context claude-code --open-web-dashboard False`.
- Serena ops: missing target in `Active language servers` → add to `language_servers:` (first = fallback), restart, verify by symbol call. `replace_symbol_body` spans leading doc + outer `#[...]` attrs → include them or use `replace_content`. Missing/new `project.yml` keys trigger a full annotated rewrite; values persist → verify `language_servers:`/`ignored_paths:` + track the template. Serena owns `.serena/.gitignore` → commit `.serena/`; root `.gitignore` = project rules. Diagnostics: Serena-served formats require explicit `get_diagnostics_for_file`; `global` plugin formats push passively.
...
- Headroom reads = lossy/reversible. `[N … hash=…]` → `mcp__headroom__headroom_retrieve`; hashless `[N lines omitted: …]` → narrow `Read`/`grep -n`. Treat compressed output as browse context. Byte-exact `Edit.old_string` → retrieve/narrow Read/grep/sed/`read_text()`, anchored on short distinctive bytes; compression rewraps prose and can mislead with `\uXXXX` hints. Store TTL = 86400s (`~/.profile`); expired/evicted hash 404 → narrow reads/Python.
...
CC maps `grep`→`rg-fff` (argv0 ugrep): default fff RE2, relevance-ranked + marked. Serving requires repo cwd; other contexts defer to ugrep. `rg`=`/usr/bin/rg`. Per-repo daemon auto-spawns (~30m idle). Fresh writes can lag its index → use `command grep` or re-query. RTK passes `grep`; rewrites `find`→`rtk find`.
...
- **Served**: literals incl. phrases w/ space/`:`/quote (regex-escaped), RE2 regex (`\b \d \w + * ( ) | {} ?`, empty-capable `x?` OK), `-i`, `-A/-B/-C N`, `-l`, `-c`, `--include=*.ext`, reldirs + `.`. Directory mode: `-r` → recursive; default → top-level.
...
- tokensave surface = MCP `mcp__tokensave__*` + CLI `init`/`sync` (re-index) + `status` (inspect). `doctor`/`install`/`reinstall` rewrite `~/.claude/settings.json` with 3 hooks + 81 untracked tool allows; `doctor` also mutates. Most subcommands' "81 tool(s) not yet permitted" warning = expected/informational.

## Subagents

- Claude usage-limit/credit signal → CLOSE-FAST: the main session is the sole Claude-credit drain (subagents run gpt-5.6-sol → separate ledger, no Claude burn) → wind it down: one summary + scoped commit, surface pending tasks. `TaskStop` any subagents still running.
