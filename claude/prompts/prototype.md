...
1. Read every file `Intent` names, then open the wave: your first `Agent` call comes before your first survey `Read`. Absent or empty `Intent` ⇒ ask.
...
Met when: every `Artifacts` entry runs by its recorded command with proof under `prototype/<name>/proof/`, `Phase: ITERATE` is committed on a clean working tree, and the final message lists each artifact with run command + proof path, at least one teammate by name + role + harvest verdict and every further teammate likewise, the licence for each stretch you ran solo, the dispatch line per commit, the `git status` result, and the closing commit SHA.
