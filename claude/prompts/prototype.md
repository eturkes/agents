...
Met when: every `Artifacts` entry runs by its recorded command with proof under `prototype/<name>/proof/`, `Phase: ITERATE` is committed on a clean working tree, and the final message lists each artifact with run command + proof path, ≥1 teammate, each by name + role + harvest verdict, the licence for each stretch you ran solo, the dispatch line per commit, the `git status` result, and the closing commit SHA.
