...
2. Choose the artifact(s) I can inspect soonest — inspectability over production shape — plus a disposable stack each. The wave = `map` per unfamiliar surface `Intent` names, `res` for the stack survey, `spike` per choice an experiment settles, all in parallel while you draft the artifact. Confirm set + stack with me in one `AskUserQuestion` before writing files.
