...
Met when: the full gate command (scanners included) passes on a clean working tree at the closing commit, CI + scanning + update automation are committed, every `Decisions` entry is implemented or moved to `Deferred` with its reason, `.agent/review.md` holds no open row, `prototype/` is retired, `Phase: MAINTAIN` is committed, and the final message states the gate command + result, the CI/scanner file paths, the dispatch line per unit, the `git status` result, and the closing commit SHA.
