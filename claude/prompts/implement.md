...
1. Stack: open the wave before your first survey `Read` — `res` per open tooling question, `spike` per choice an experiment settles, `map` per unfamiliar surface, all in parallel; select for task + agent fit + SOTA over popularity; my preselection wins. One `AskUserQuestion` for the stack + every spec gap before files that encode them; ask again whenever direction is unclear.
...
Met when: the full gate command (scanners included) passes on a clean working tree at the closing commit, CI + scanning + update automation are committed, every `Decisions` entry is implemented or moved to `.agent/deferred.md` with its reason, `.agent/review.md` holds no open row, `prototype/` is retired, `Phase: MAINTAIN` is committed, and the final message states the gate command + result, the CI/scanner file paths, at least one teammate by name + role + harvest verdict and every further teammate likewise, the licence for each unit you ran solo, the dispatch line per unit, the `git status` result, and the closing commit SHA.
