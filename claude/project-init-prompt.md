...
6. Minimum runnable skeleton: package metadata carrying the project name and the `LICENSE` identifier, one entry point, one test that proves the harness runs, and every gate command wired. Run each gate once; record its exact invocation + result in `.agent/memory.md`. Features belong to milestones, not here.
